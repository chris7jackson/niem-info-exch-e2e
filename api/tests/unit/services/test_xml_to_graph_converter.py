        ...
        "objects": [
            {
                "qname": "nc:Location",
                "label": "Location",
                "properties": {"nc:LocationStateFIPS5-2AlphaCode": "{{text}}"},
            }
        ],
        "associations": [],
        "references": [],
...
<nc:Location xmlns:nc="http://example.com/nc">
    ...
    # Generate Cypher
    cypher, _, _, _ = generate_for_xml_content(xml_str, mapping_dict, "test.xml")

    # Verify that the hyphenated property name is escaped with backticks
    assert "`nc_LocationStateFIPS5-2AlphaCode`" in cypher, "Property name with hyphen should be escaped with backticks"

    # Verify there's no unescaped version that would cause syntax error
    assert (
        "n.nc_LocationStateFIPS5-2AlphaCode=" not in cypher or "`nc_LocationStateFIPS5-2AlphaCode`" in cypher
...
def test_property_name_with_dot_escaping():
    ...
    mapping_dict = {
        "objects": [{"qname": "nc:Person", "label": "Person", "properties": {"nc:Person.Name": "{{text}}"}}],
        "associations": [],
        "references": [],
        "namespaces": {"nc": "http://example.com/nc"},
    }
...
<nc:Person xmlns:nc="http://example.com/nc">
    ...
    assert (
        "`nc_Person.Name`" in cypher or "`nc_Person_Name`" in cypher
...
def test_normal_property_name_no_escaping():
    ...
    mapping_dict = {
        "objects": [{"qname": "nc:Person", "label": "Person", "properties": {"nc:PersonName": "{{text}}"}}],
        "associations": [],
        "references": [],
        "namespaces": {"nc": "http://example.com/nc"},
    }
...
<nc:Person xmlns:nc="http://example.com/nc">
    ...
    assert "nc_PersonName" in cypher, "Normal property names should be present in output"
...
def test_synth_id_uses_sha1():
    ...
    result = synth_id(
        parent_id="parent_123", elem_qn="nc:Person", ordinal_path="/root[1]/person[2]", file_prefix="abc123"
    )
