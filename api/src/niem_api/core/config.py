...
class BatchConfig:
    ...
    def __init__(self):
        ...
        self.MAX_CONCURRENT_OPERATIONS = int(os.getenv('BATCH_MAX_CONCURRENT_OPERATIONS', '3'))

        # Timeout: Max seconds per individual file operation
        self.OPERATION_TIMEOUT = int(os.getenv('BATCH_OPERATION_TIMEOUT', '60'))

        # Operation-specific batch size limits
        # Schema uploads often have 25+ XSD files (NIEM references)
        self.MAX_SCHEMA_FILES = int(os.getenv('BATCH_MAX_SCHEMA_FILES', '150'))

        # XML/JSON conversions typically smaller batches
        self.MAX_CONVERSION_FILES = int(os.getenv('BATCH_MAX_CONVERSION_FILES', '150'))

        # XML/JSON ingestion to Neo4j
        self.MAX_INGEST_FILES = int(os.getenv('BATCH_MAX_INGEST_FILES', '150'))

    def get_batch_limit(self, operation_type: str) -> int:
        ...
        """
        limits = {
            'schema': self.MAX_SCHEMA_FILES,
            'conversion': self.MAX_CONVERSION_FILES,
            'ingest': self.MAX_INGEST_FILES,
        }
